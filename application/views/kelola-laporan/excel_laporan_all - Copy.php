<?php 
 ...
 header("Content-Disposition: attachment; filename=Laporan_Pemeriksaan".$jenis.".xls");
          ...
          <tr>
           ...
           <?php 
            ...
            foreach ($record as $row) {
              $tul = $this->db->query("SELECT * FROM tb_tl WHERE rekomendasi_id = '$row[rekomendasi_id]'")->result_array(); 
              $hitung[] = count($tul);
            } 
            // print_r($hitung);
            if (!empty($hitung)) {
          ...
          <tr>
            ...
            ?></td>
              <?php $useropr = $this->session->username;
                    ...
                    if ($this->session->level=="spi" OR $this->session->level=="kabagspi" OR $this->session->level=="admin") {
                      ...
                      $tl = $this->db->query("SELECT * FROM tb_tl WHERE rekomendasi_id = '$value[rekomendasi_id]'")->result_array();
                    }else{
                      $tl = "<center>-</center>";
                    }
          ...
          <?php
            ...
            ?></td>
              <?php $useropr = $this->session->username;
                    ...
                    if ($this->session->level=="spi" OR $this->session->level=="kabagspi" OR $this->session->level=="admin") {
                      $tl = $this->db->query("SELECT * FROM tb_tl WHERE rekomendasi_id = '$value[rekomendasi_id]' AND tl_publish_spi='Y'")->result_array();
                    }elseif($this->session->level=="operator" OR $this->session->level=="viewer" OR $this->session->level=="verifikator"  OR $this->session->level=="administrasi"){
                      $tl = $this->db->query("SELECT * FROM tb_tl WHERE rekomendasi_id = '$value[rekomendasi_id]'")->result_array();
                    }else{
                      $tl = "<center>-</center>";
                    }
