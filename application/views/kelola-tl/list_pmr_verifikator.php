                     ...
                     <div class="panel">
                      ...
                      <div id="collapseTwo<?php echo $no; ?>" class="panel-collapse collapse" role="tabpanel" aria-labelledby="headingTwo<?php echo $no; ?>" style="padding: 5px 0px 5px 20px" >
                        ...
                        foreach($tmn as $key => $value){ 
                          ...
                          //   rekomendasi_kirim','Y');
                            $q = $this->db->query("SELECT * FROM `tb_rekomendasi` WHERE `temuan_id` = $value[temuan_id] AND `rekomendasi_kirim` = 'Y'")->result_array();
                            $unit=$this->session->unit;
 
                            if($q[0]['unit_id'] == $this->session->unit){
                              $rkm = $this->db->query("SELECT * FROM `tb_rekomendasi` WHERE `temuan_id` = $value[temuan_id] AND `rekomendasi_kirim` = 'Y' AND unit_id=$unit")->result_array();
                            }
                            else{
                              $rkm = $this->db->query("SELECT * FROM `tb_rekomendasi` WHERE `temuan_id` = $value[temuan_id] AND `rekomendasi_kirim` = 'Y' AND unit_id=$unit")->result_array();
                            }
